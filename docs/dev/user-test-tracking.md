...
| UT-CORNER-001 | Retest keyed fillet/chamfer on dimensioned rectangles: radius/bridge and existing length dimensions should move during edge drag and keep their displayed values. Also retest the still-open pre-confirm size preview and dimensions attached directly to the edge being trimmed. |
...
| UT-SOLVER-008 | Add an angular dimension between two converging lines on the obtuse side; it should read the supplementary angle (for example `135`, not `45`) and dragging the dimension label across the acute side should not flip the stored/readout angle. |
...
| UT-SOLVER-007 | After editing a driving dimension value, dragging/rotating the associated geometry keeps dimension graphics/anchors traveling with that geometry. |
...
| ID | Behavior | User Status | Automated Status | App Status | Current Stage |
|---|---|---|---|---|---|
| UT-CORNER-001 | Fillet/Chamfer activate without valid preselection, accept exactly two converging edges or one two-edge vertex, preserve constraints, use shortest fillet sweep, and support keyed size plus drag/post-create dimension adjustment. | USER-FAILED | AUTO-PASSED | PARTIAL | Corner resolver, modal pre/post selection, keyed tool-size field, stale-corner constraint replacement, shortest fillet sweep, generated dimensions, keyed Equal chains, and dimensioned fillet/chamfer drag are coded. Running-app drag checks passed for radius/bridge and length dimensions moving with dragged geometry. Still open: interactive post-selection size preview before confirm and dimensions directly attached to the trimmed edge refreshing to shortened segment length |
...
| UT-SOLVER-007 | Persistent dimension graphics/anchors travel with geometry after the driving dimension value has been edited. | USER-FAILED | AUTO-PASSED | APP-VERIFIED | Dimension anchors and canvas-point dimension references now transform with selected/document translate and rotate paths. Rebuilt-app browser verification confirmed a keyed 10-unit line dimension followed Translate by `+96,-48` screen px and Rotate to the expected `391.2,310` anchor while keeping value `10`; exact user rectangle workflow still worth retesting |
| UT-SOLVER-008 | Angular dimensions respect selected supplementary angles and do not flip the displayed/stored span when the label crosses the acute side. | USER-FAILED | AUTO-PASSED | APP-VERIFIED | Coded and app-verified: two converging lines produced a persistent Angle input of `135` from the obtuse-side placement, then dragging the dimension label into the acute-side area kept value/rendered span at `135`; awaiting user retest |
