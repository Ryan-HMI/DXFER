...
| ID | Behavior | User Status | Automated Status | App Status | Current Stage |
|---|---|---|---|---|---|
| UT-CORNER-001 | Fillet/Chamfer activate without valid preselection, accept exactly two converging edges or one two-edge vertex, preserve constraints, use shortest fillet sweep, and support keyed size plus drag/post-create dimension adjustment. | USER-FAILED | AUTO-PASSED | CODED | Core corner resolver, modal pre/post selection, keyed tool-size field, stale-corner constraint replacement, shortest fillet sweep, generated fillet/chamfer dimensions, fillet radius drag, and chamfer bridge drag-as-size-adjustment coverage are coded; needs running-app/user retest |
