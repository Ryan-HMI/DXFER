...
public static class LineSplitService
{
        ...
        out DrawingDocument nextDocument)
    {
        ...
        foreach (var entity in document.Entities)
        {
            ...
            {
                nextEntities.Add(new LineEntity(line.Id, line.Start, splitPoint));
                nextEntities.Add(new LineEntity(newLineId, splitPoint, line.End));
                split = true;
                continue;
            }
        ...
        nextDocument = split
            ? new DrawingDocument(nextEntities)
            : document;
        return split;
    }
